...
class _OrderDetailsState extends State<OrderDetails> {
  ...
  fetchTrackOrderDetails() async {
    _trackingList.clear();
    var trackOrderDetailsResponse = await OrderRepository().getTrackOrder(
        courierCode: _orderDetails!.carrier!.code,
        resiNumber: _orderDetails!.tracking_code);
    if (trackOrderDetailsResponse.rajaongkir.result.manifest.length > 0) {
      List<dynamic> srcTrackingList = [];
      srcTrackingList
          .addAll(trackOrderDetailsResponse.rajaongkir.result.manifest);
      _trackingList = srcTrackingList.reversed.toList();
          ...
          return Wrap(
            children: [
              Center(
                ...
                child: Text(
                  LangText(context).local.tracking_detail_ucf,
                  style: TextStyle(
                      fontSize: 15,
      ...
      ToastComponent.showDialog(LangText(context).local.tracking_alert_ucf,
          gravity: ToastGravity.CENTER, duration: Toast.LENGTH_LONG);
    }
    setState(() {});
  }
  ...
  _onPressReceiveOrder(orderId) async {
    Loading.show(context);
    var response = await OrderRepository().receiveOrder(orderId);
    Loading.close();
    if (response.result) {
      var response = await OrderRepository().updatePaymentOrder(orderId);
      if (response.result) {
        _onPageRefresh();
      }
    }
                ...
                SliverToBoxAdapter(
                  child: Padding(
                      padding: const EdgeInsets.only(
                          left: 10.0, right: 10.0, bottom: 10.0),
                      child: _orderDetails != null
                          ? buildShowTrackingOrder(context)
                          : buildTimeLineShimmer()),
                ),
                SliverList(
          ...
          children: [
              ...
              child: Row(
                children: [
                  GestureDetector(
                    onTap: () {
                      Clipboard.setData(
                          ClipboardData(text: _orderDetails!.code!));
                      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
                        content: Text("Kode pesanan telah disalin"),
                      ));
                    },
                    child: Wrap(
            ...
            Row(
              children: [
                ...
                Spacer(),
                Text(
                  AppLocalizations.of(context)!.payment_code_ucf,
                  style: TextStyle(
                      color: MyTheme.font_grey,
                      fontSize: 13,
                      fontWeight: FontWeight.w600),
            ...
            Padding(
              ...
              child: Row(
                children: [
                  ...
                  Spacer(),
                  _orderDetails!.payment_type == "Wallet"
                      ? Text(
                          "-",
                          style: TextStyle(
                            color: MyTheme.grey_153,
                      ...
                      : GestureDetector(
                          onTap: () {
                            String srcCode = _orderDetails!.code!;
                            Clipboard.setData(ClipboardData(
                                text: srcCode.split("-").last.trim()));
                          ...
                          child: Wrap(
                            ...
                            children: [
                              Text(
                                _orderDetails!.code!.split("-").last.trim(),
                                style: TextStyle(
                                    color: MyTheme.accent_color,
                                    fontSize: 14,
                                    fontWeight: FontWeight.w600),
                  ...
                  _orderDetails!.shipping_address != null
                      ? Column(
                          ...
                          children: [
                            ...
                            Text(
                              "${AppLocalizations.of(context)!.sub_district_ucf}: ${_orderDetails!.shipping_address!.sub_district}",
                              maxLines: 3,
                              style: TextStyle(
                                color: MyTheme.grey_153,
                              ),
                            ...
                            Text(
                              "${AppLocalizations.of(context)!.province_ucf}: ${_orderDetails!.shipping_address!.province}",
                              maxLines: 3,
                              style: TextStyle(
                                color: MyTheme.grey_153,
                              ),
  ...
  Widget buildShowReceiveOrder(BuildContext context) {
    if (_orderDetails?.delivery_status != "on_the_way") {
      ...
      child: Row(
        ...
        children: [
          Btn.basic(
            minWidth: 60,
            onPressed: () {
              _onPressReceiveOrder(_orderDetails?.id.toString());
            },
            child: Container(
              ...
              child: Text(
                LangText(context).local.received_order_ucf,
                style: TextStyle(
                  color: MyTheme.grey_153,
                  fontSize: 14,
                ),
          ...
          const SizedBox(width: 16), // Padding kanan (opsional)
        ],
      ),
    );
  }

  Widget buildShowTrackingOrder(BuildContext context) {
    return Container(
      height: 40,
      child: Center(
        child: InkWell(
          onTap: () {
            if (_orderDetails!.tracking_code != null) {
              ...
              ToastComponent.showDialog(
                  "Lacak belum tersedia, resi belum diinput seller.",
          ...
          child: Padding(
            ...
            child: Text(
              LangText(context).local.tracking_order_ucf,
              style: TextStyle(
                  fontSize: 14,
                  decoration: TextDecoration.underline,
                  color: MyTheme.accent_color),
  ...
  Widget buildTrackDetailSection() {
          ...
          return Column(
            ...
            children: [
              Padding(
                padding: const EdgeInsets.only(bottom: 8.0),
                child: Text(
                  _trackingList[index].manifestDescription,
                  style: index == 0
                      ? TextStyle(
                          color: Colors.black,
                          fontSize: 13,
              ...
              Padding(
                ...
                child: Text(
                    _trackingList[index].manifestDate +
                        " | " +
                        _trackingList[index].manifestTime,
                    style:
